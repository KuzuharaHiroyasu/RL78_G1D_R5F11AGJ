...
#define		DRV_FRAM_WAIT_THIGH				WAIT_2US();														/* データクロック"H"時間(汎用)		*/
#define		DRV_FRAM_WAIT_TLOW				WAIT_2US();														/* データクロック"L"時間(汎用)		*/
#define		DRV_FRAM_WAIT_TSU_STA			WAIT_2US();					/* スタートコンディションセットアップ時間	*/
#define		DRV_FRAM_WAIT_THD_STA			WAIT_2US();					/* スタートコンディションホールド時間		*/
#define		DRV_FRAM_WAIT_TSU_STO			WAIT_2US();					/* ストップコンディションセットアップ時間	*/
#define		DRV_FRAM_WAIT_THD_STO			WAIT_2US();					/* ストップコンディションホールド時間 ※マニュアルに記載無し */

#define		DRV_FRAM_SCL_LOW				PORT_OUTPUT(DRV_FRAM_P_SCL ,LOW  );				/* SCL Low	 */
#define		DRV_FRAM_SCL_HIGH				PORT_OUTPUT(DRV_FRAM_P_SCL ,HIGH );				/* SCL High	 */
#define		DRV_FRAM_SDA_LOW				PORT_DIRECTION_OUTPUT_SETTING(DRV_FRAM_PD_SDA);PORT_OUTPUT(DRV_FRAM_P_SDA ,LOW  );				/* SDA Low	 */
#define		DRV_FRAM_SDA_HIGH				PORT_DIRECTION_INPUT_SETTING(DRV_FRAM_PD_SDA);	/* SDA High ※入力モードで、ハードのプルアップによる"H"を作成 */

#define		DRV_FRAM_SCL_LOW				( P6  &= ~0x01 );
#define		DRV_FRAM_SCL_HIGH				( P6  |= 0x01 );
#define		DRV_FRAM_SDA_LOW				( PM6 &= ~0x02);(P6 &= ~0x02);
#define		DRV_FRAM_SDA_HIGH				( PM6 |= 0x02);

#define DRV_FRAM_RESET_CNT			9						/* スタートコンディション回数[9] */
...
static void R_IICA0_Reset(void)
{
	...
	// ダミークロック9回
	for( i = 0; i < DRV_FRAM_RESET_CNT; i++ ){
		DRV_FRAM_SCL_LOW;
		DRV_FRAM_WAIT_TLOW;
		DRV_FRAM_SCL_HIGH;
		DRV_FRAM_WAIT_THIGH;
...
static void R_IICA0_Reset_Start(void)
{
	DRV_FRAM_SCL_LOW;
	DRV_FRAM_SDA_HIGH;
	DRV_FRAM_WAIT_TLOW;			/* データクロック"L"時間					*/

	DRV_FRAM_SCL_HIGH;
	DRV_FRAM_WAIT_TSU_STA;			/* スタートコンディションセットアップ時間	*/

	DRV_FRAM_SDA_LOW;				/* これがSTART BIT						*/
	DRV_FRAM_WAIT_THD_STA;			/* スタートコンディションホールド時間		*/
...
static void R_IICA0_Reset_Stop(void)
{
	...
	DRV_FRAM_SCL_HIGH;
	DRV_FRAM_WAIT_TSU_STO;		/* ストップコンディションセットアップ時間	*/

	DRV_FRAM_SDA_HIGH;			/* これがSTOP BIT						*/
	DRV_FRAM_WAIT_THD_STO;		/* ストップコンディションホールド時間	*/
