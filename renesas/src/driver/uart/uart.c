...
_UARTCODE void serial_init(SERIAL_EVENT_PARAM *param)
{
	...
	//RD8001‘Î‰ž:’ÊMÝ’è•ÏX
	#if 0
    /* Serial communication operation settings */
    write_sfrp(UART_TXD_SCR, (uint16_t)0x8097U);		// 8bit,ƒpƒŠƒeƒB‚È‚µ,LSB
    write_sfrp(UART_RXD_SCR, (uint16_t)0x4497U);		// 8bit,ƒpƒŠƒeƒB‚È‚µ,LSB
    #else
    /* Serial communication operation settings */
    write_sfrp(UART_TXD_SCR, (uint16_t)0x8297U);		// 8bit,‹ô”ƒpƒŠƒeƒB,LSB
    write_sfrp(UART_RXD_SCR, (uint16_t)0x4697U);		// 8bit,‹ô”ƒpƒŠƒeƒB,LSB
    #endif
